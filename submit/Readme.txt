...
POS Tagging Result:
	- 3_2_POS_tagging_result.txt (https://www.dropbox.com/s/oxvpfjluiru5pcz/3_2_POS_tagging_result.txt?dl=0)
		This file contains the result for 3.2 POS tagging.

	- 3_4_POS_tagging_result.txt (https://www.dropbox.com/s/4ismizb6iykvo6c/3_4_POS_tagging_result.txt?dl=0)
		This file contains the result for 3.4 POS Tagging.

File Hierarchy:
	- collect_data.py 
		This file is to collect and convert data from the original "Posts.xml" to several Pandas Dataframe and then store them in the csv format for further processing. 
		The output consists 3 files: "questions.csv" including all the question posts, "answers.csv" including all the answers posts, "code.csv" including code blocks in each post accordingly.

	- stemmer.py
		This file is a stemmer that takes in all the selected posts and perform stemming on each token. 
		The output is a report for word frequency analysis.

	- pos_tagging.py
		This file is a simple tool for pos tagging. It takes a sentence as input.
		The output is the corresponding pos tagging result.

	- tokenizer.py
		This file is the tokenizer we implemented for this project. The tokenizer will take code.csv, questions.csv and answers.csv (generated by collect_data.py) together with the test set used for hand annotation and testing. 
		The output contains 3 parts: result for the whole dataset, result for each post from test set, result for overall test set. 

	- tester.py
		This file is for testing our generated tokens by comparing with the hand annotation one. It takes two directories as input, one is the generated tokens and the other is the hand annotated tokens. 
		The output is a report that contains all the wrong tokens and the total number of mistakes.
