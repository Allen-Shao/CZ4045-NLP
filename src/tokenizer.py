...
parser.add_argument("code", help="path of code.csv")
...
parser.add_argument("ground_truth", help="path of 150_ground_truth_post.csv")
args = parser.parse_args()

ALL_codes = pd.read_csv(args.code)
ALL_codes.set_index('PostId', inplace=True)

selected_qns = pd.read_csv(args.selected_questions)
selected_ans = pd.read_csv(args.selected_answers)
selected_post = pd.concat([selected_ans, selected_qns])[['PostId', 'Body']]
selected_post.set_index('PostId', inplace=True)

ground_truth_post = pd.read_csv(args.ground_truth)
